...
gulp.task('copy', function() {
    return gulp.src(['**/*', '!**/*.ts'], { base: 'src' }).pipe(gulp.dest(dist));
});
var cwd = process.cwd();
...
mtasks.forEach(function(pname) {
    ...
    if (fs.existsSync(`${ppath}/miniprogram_dist`)) {
        ...
        gulp.task(pname, function() {
            return gulp.src(`${dist_dir}/**/*`).pipe(gulp.dest(`${dist}/miniprogram_npm/${pname}`));
        });
    } else {
...
gulp.task('watch', function() {
    gulp.watch('src/**/*').on('change', function(file) {
        ...
        if (parsed.ext === '.ts') {
            ...
            gulp.src(`**/${src}`, { base: 'src' }).pipe(gulp.dest(dist));
        }
    });
});
gulp.task('default', gulp.series('clean', 'build', 'copy', gulp.parallel(mtasks), 'watch'));
gulp.task('nowatch', gulp.series('clean', 'build', gulp.parallel(mtasks), 'copy'));
